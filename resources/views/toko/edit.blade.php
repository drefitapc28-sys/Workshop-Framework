...
<style>
    ...
    .btn-save { background: linear-gradient(135deg, #2563eb, #1d4ed8); color: white; border: none; border-radius: 10px; padding: 11px 28px; font-weight: 700; }
    ...
    .coord-display { background: #f8fafc; border: 2px solid #e2e8f0; border-radius: 10px; padding: 12px 14px; font-family: monospace; font-size: .9rem; }
    .acc-badge { background: #dbeafe; color: #2563eb; padding: 4px 12px; border-radius: 20px; font-size: .85rem; font-weight: 600; }
</style>

<div class="form-wrap px-3">
    <div class="page-header">
        <h5 class="mb-1 fw-bold"><i class="bi bi-pencil-square me-2"></i>Edit Toko</h5>
        <small class="opacity-75">Perbarui data: {{ $toko->nama_toko }}</small>
    </div>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('toko.update', $toko->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="alamat" class="form-control" rows="2">{{ old('alamat', $toko->alamat) }}</textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div>
                        ...
                        <input type="number" name="accuracy" id="accuracy" class="form-control"
                               value="{{ old('accuracy', $toko->accuracy) }}" step="any" required>
                    </div>
                </div>

                <div class="coord-display mb-4">
                    <i class="bi bi-geo-fill text-primary me-2"></i>
                    <span id="coordText">{{ $toko->latitude }}, {{ $toko->longitude }}</span>
                    <span class="ms-2 acc-badge" id="accBadge">±{{ $toko->accuracy }}m</span>
                </div>

                <div class="d-flex gap-2">
                    <button type="submit" class="btn-save">
                        <i class="bi bi-check-lg me-1"></i> Update Toko
                    </button>
...
function ambilLokasi() {
    ...
    status.innerHTML = '<span class="text-primary"><span class="spinner-border spinner-border-sm me-1"></span>Mengambil...</span>';
    navigator.geolocation.getCurrentPosition(
        function(pos) {
            ...
            document.getElementById('accBadge').textContent  = '±' + Math.round(acc) + 'm';
            status.innerHTML = '<span class="text-success"><i class="bi bi-check-circle me-1"></i>Lokasi diperbarui</span>';
        },
        function() {
            status.innerHTML = '<span class="text-danger"><i class="bi bi-exclamation-circle me-1"></i>Gagal mengambil lokasi</span>';
        },
