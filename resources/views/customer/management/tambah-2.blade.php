        ...
        <div class="col-md-8 offset-md-2">
            ...
            <div class="card shadow-sm">
                <div class="card-body">
                    ...
                    <div class="alert alert-success mb-4">
                        ...
                        <strong>Metode Penyimpanan:</strong> Foto akan disimpan ke storage/public/customers/ dan path disimpan di database
                    </div>

                    <!-- Form -->
                    <form id="formCustomer2" onsubmit="submitForm(event)">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <div class="btn-group d-flex gap-2 mb-3">
                                <button type="button" id="startCamera2" class="btn btn-primary flex-grow-1">
                                    <i class="mdi mdi-play"></i> Mulai Kamera
                                </button>
                                <button type="button" id="capturePhoto2" class="btn btn-success flex-grow-1">
                                    <i class="mdi mdi-camera"></i> Ambil Foto
                                </button>
                                <button type="button" id="stopCamera2" class="btn btn-danger flex-grow-1">
                        ...
                        <div id="previewSection" style="display:none;" class="mb-3">
                            ...
                            <div class="text-center">
                                <img id="photoPreview2" class="img-thumbnail rounded" style="max-width: 100%; max-height: 300px;">
                            </div>
                        ...
                        <input type="hidden" name="foto_file" id="fotoFileInput2">
                        ...
                        <div class="d-grid gap-2 mb-3">
                            <button type="submit" class="btn btn-lg btn-success" id="submitBtn">
                                <i class="mdi mdi-content-save"></i> Simpan Customer (FILE)
                            </button>
                        </div>
            ...
            <div class="alert alert-warning mt-4">
                ...
                <ul class="mb-0 mt-2">
                    ...
                    <li>Foto akan disimpan ke storage publik (dapat diakses via URL)</li>
                </ul>
            </div>
        </div>
    </div>
...
<script src="{{ asset('js/camera-file.js') }}"></script>

<script>
async function submitForm(event) {
    event.preventDefault();

    const fotoFileInput = document.getElementById('fotoFileInput2').value;
    if (!fotoFileInput) {
        alert('Foto belum diambil! Silakan klik "Ambil Foto" terlebih dahulu.');
        return;
    }

    const formData = new FormData(document.getElementById('formCustomer2'));

    document.getElementById('loadingSpinner').style.display = 'block';
    document.getElementById('submitBtn').disabled = true;

    try {
        const response = await fetch('{{ route("customer.management.store2") }}', {
            method: 'POST',
            body: formData,
            headers: {
